...
**禁止事項:**
- ❌ 500件を超えるfiltered_companies.ndjsonを読み込むこと → タイムアウトリスクあり
- ❌ Taskツールで並列処理 → 今回は不要（filtered_companies.ndjsonは十分小さい）
- ❌ 親ディレクトリ（../）へのアクセス

**必須事項:**
- ✅ Step 1: Bashでgrepフィルタリング → 500件以下に制限
- ✅ Step 2: `output/{ulid}/chunks/filtered_companies.ndjson` をReadツールで読み込む
- ✅ Step 3: 上位{count}社を選出して評価
- ✅ **Step 4: 必ず最終成果物を作成** → `output/{ulid}/companies_summary.md` と `output/{ulid}/companies.csv`
- ✅ 作業ディレクトリ: `workspace/` 内のみ

**⚠️ 重要: 最終ファイルを必ず作成してください**
処理が完了したら、必ずWriteツールで以下の2ファイルを作成すること：
1. `output/{ulid}/companies_summary.md`
2. `output/{ulid}/companies.csv`
