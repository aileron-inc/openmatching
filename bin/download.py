...
def filter_candidates(df, recent_interview_days=60, min_survey_year=2024, valid_ranks=['S', 'A', 'B']):
    """求職者データをフィルタリング"""
    ...
    if rank_col in df.columns:
        ...
        print(f"  ランクフィルタ ({', '.join(valid_ranks)}): {len(df)}件 ({before - len(df)}件除外)")
    ...
    if survey_col in df.columns:
        ...
        print(f"  アンケート日フィルタ ({min_survey_year}年以降): {len(df)}件 ({before - len(df)}件除外)")
    ...
    if interview_col in df.columns:
        ...
        active_statuses = ['書類選考中', '一次面接中', '二次面接中', '最終面接中', 'オファー面談中']
        ...
        if status_col in df.columns:
            ...
            print(f"  面談日({recent_interview_days}日以内) OR 選考中: {len(df)}件 ({before - len(df)}件除外)")
...
def filter_jobs(df, job_status='アクティブ'):
    ...
    if status_col:
        ...
        print(f"  求人状態フィルタ ({job_status}): {len(df)}件 ({before - len(df)}件除外)")
