...
# .env を読み込む (.envがあれば読み込む。既存の環境変数は上書きしない)
...
def check_env_vars():
    ...
    for var_name, var_desc in required_vars.items():
        ...
        if value:
            ...
            results.append({"status": "missing", "name": var_name, "desc": var_desc})
...
def test_opencode():
    """OpenCode実行テスト"""
    test_prompt = (
        "1から5までの数字をカンマ区切りで出力してください。他の説明は不要です。"
    )
...
def main():
    ...
    parser.add_argument(
        "--test-opencode", action="store_true", help="OpenCode実行テスト"
    )
